...
export default function Index({ transactions, stats, filters }) {
    ...
    const getStatusStyle = (status) => {
        switch (status) {
            case 'Completed':
                return { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-700 dark:text-green-400', dot: 'bg-green-600' };
            case 'Processing':
                return { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-700 dark:text-yellow-400', dot: 'bg-yellow-600' };
            case 'Refund':
                return { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-700 dark:text-red-400', dot: 'bg-red-600' };
            default:
                return { bg: 'bg-gray-100 dark:bg-white/10', text: 'text-gray-700 dark:text-gray-300', dot: 'bg-gray-600' };
        }
    };
        ...
        <AdminLayout currentRoute="transaction">
            ...
            <div className="flex justify-between items-end mb-8">
                ...
                <div className="flex items-center space-x-3">
                    <div className="flex items-center bg-white dark:bg-black border border-gray-200 dark:border-white/10 rounded-lg p-1 shadow-sm">
                        <input 
                            type="date" 
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div className="bg-white dark:bg-black p-6 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm flex flex-col justify-between h-32 transition-colors">
                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 tracking-widest uppercase">Payments</p>
                    <div className="flex items-end justify-between">
                        <h3 className="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">{formatCurrency(stats.total_payments)}</h3>
                ...
                <div className="bg-white dark:bg-black p-6 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm flex flex-col justify-between h-32 transition-colors">
                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 tracking-widest uppercase">Completed</p>
                    <div className="flex items-end justify-between">
                        <h3 className="text-4xl font-extrabold text-green-600 dark:text-green-400 tracking-tight">{stats.completed}</h3>
                        <span className="inline-flex items-center px-2 py-1 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 text-[10px] font-bold rounded uppercase">Verified</span>
                    </div>
                </div>

                <div className="bg-white dark:bg-black p-6 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm flex flex-col justify-between h-32 transition-colors">
                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 tracking-widest uppercase">Processing</p>
                    <div className="flex items-end justify-between">
                        <h3 className="text-4xl font-extrabold text-yellow-600 dark:text-yellow-400 tracking-tight">{stats.processing}</h3>
                        <span className="inline-flex items-center px-2 py-1 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-600 dark:text-yellow-400 text-[10px] font-bold rounded uppercase">On Hold</span>
                    </div>
                </div>

                <div className="bg-white dark:bg-black p-6 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm flex flex-col justify-between h-32 transition-colors">
                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 tracking-widest uppercase">Refund</p>
                    <div className="flex items-end justify-between">
                        <h3 className="text-4xl font-extrabold text-red-600 dark:text-red-400 tracking-tight">{stats.refund}</h3>
                        <span className="inline-flex items-center px-2 py-1 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-[10px] font-bold rounded uppercase">Returned</span>
                    </div>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                            {transactions.data.map((transaction) => {
                                ...
                                return (
                                    <tr key={transaction.pembelian_id} className="hover:bg-gray-50/30 dark:hover:bg-white/5 transition-colors">
                                        <td className="px-6 py-5">
                                            <span className="text-xs font-mono font-bold text-gray-900 dark:text-white">#TM-{transaction.pembelian_id.toString().padStart(5, '0')}</span>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex items-center">
                                                <div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-white/10 flex items-center justify-center mr-3 border border-gray-200 dark:border-white/5">
                                                    <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400">{transaction.user?.user_fullname.substring(0, 2).toUpperCase()}</span>
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-gray-900 dark:text-white leading-none">{transaction.user?.user_fullname}</p>
                                                    <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">{transaction.user?.user_email}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <p className="text-xs font-bold text-gray-900 dark:text-white">{dayjs(transaction.pembelian_tanggal).format('MMM DD, YYYY')}</p>
                                            <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">{dayjs(transaction.pembelian_tanggal).format('HH:mm A')}</p>
                                        </td>
                                        <td className="px-6 py-5">
                                            <span className="text-sm font-black text-gray-900 dark:text-white">{formatCurrency(transaction.pembelian_total_harga)}</span>
                                        </td>
                                        <td className="px-6 py-5">
